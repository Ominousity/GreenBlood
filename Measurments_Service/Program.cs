...
MeasurmentGroup.MapGet("/Get/{SSN}", async (IMeasurmentService measurmentService, string SSN) =>
{
    return await Task.FromResult(measurmentService.GetMeasurements(SSN));
});

MeasurmentGroup.MapPost("/Add/{SSN}", async (IMeasurmentService measurmentService, Measurement measurement, string SSN) =>
{
    measurmentService.AddMeasurement(measurement, SSN);
    return Results.Created($"/Measurment/Get/{SSN}", measurement);
});

MeasurmentGroup.MapPut("/Update/{SSN}", async (IMeasurmentService measurmentService, Measurement measurement, string SSN) =>
{
